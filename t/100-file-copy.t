...
lives-ok { copy $f1, $f2; }, "copy file to existing file";
is (slurp $f2), (slurp $f1), "copied file is identical to original";

# file to non-existing file
lives-ok { copy $f1, $f3; }, "copy file to non-existing file";
is (slurp $f3), (slurp $f1), "copied file is identical to original";

# file to existing file
die "FATAL: file \$f3 does not exist" if not $f3.IO.f;
dies-ok { copy $f1, $f3, :createonly; }, "throw with :createonly and don't overwrite an existing file";

# dir to dir
# copy one dir to another
die "FATAL: dir \$tdir does not exist" if not $d1.IO.d;
die "FATAL: dir \$d1 does not exist" if not $d1.IO.d;
lives-ok { copy $tdir, $d1 }, "copy contents of one dir to another";
...
dies-ok { copy $d1, $f1; }, "throw when trying to copy dir to an existing file";
